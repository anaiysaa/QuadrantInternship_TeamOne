...
export function ManageOnboardingDialog({
  open,
  onOpenChange,
  candidate,
  refreshCandidates,
  ...
  useEffect(() => {
    ...
    // ✅ If tasks are already assigned, use them
    if (
      Array.isArray(candidate.checklistTasksStatus) &&
      candidate.checklistTasksStatus.length > 0
    ) {
      setTasks(
        candidate.checklistTasksStatus.map((task) => ({
          id: task.itemId,
          name: task.taskDesc,
      ...
      // ✅ Fallback: Try to fetch tasks from assigned checklist if available
      async function fetchTasksFromChecklist() {
        try {
          // If candidate has checklistAssigned = true, try using /checklists/:id
          if (candidate.checklistAssigned && candidate.checklistId) {
            const res = await fetch(`/onboarding/checklists/${candidate.checklistId}`);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/onboarding/newhires/${candidate.id}/checkliststatus`,
        {
          method: "PATCH",
